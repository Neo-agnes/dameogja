var SITE_SHOP_DETAIL = function(){ 
	...
	var prod_type = CONST.PROD_TYPE.NORMAL;
	...
	var prod_type_org = CONST.PROD_TYPE.NORMAL;
	...
	var default_options = {
		...
		is_site_page: false,
		prod_type: CONST.PROD_TYPE.NORMAL,
	...
	*/
	var initDetail = function(_option){
		...
		is_mobile_width = ( window_width < CONST.WIDTH_MOBILE );
			...
			if($seemore_wrap.length > 0){
				if(!is_mobile_width){
					...
					if(!use_lazy_load || $prod_detail_content_pc.find('img').length === 0){
						if ($prod_detail_content_pc.outerHeight() < CONST.SEEMORE_HEIGHT.PC && options.shop_pc_tab_type_one_page != 'Y') {
		...
		switch( hash_temp[0] ) {
			case '#prod_detail_detail':
				if(is_mobile_width) SITE_SHOP_DETAIL.changeContentTab(CONST.TAB_TYPE.DETAIL);
				else pc_tab_function(CONST.TAB_TYPE.DETAIL);
				break;
			case '#prod_detail_review':
				if(is_mobile_width) SITE_SHOP_DETAIL.changeContentTab(CONST.TAB_TYPE.REVIEW);
				else pc_tab_function(CONST.TAB_TYPE.REVIEW);
				if( hash_temp[1] ) {
					SITE_SHOP_DETAIL.viewReviewDetail(hash_temp[1], 1, 'N', 'Y');
				}
				break;
			case '#prod_detail_qna':
				if(is_mobile_width) SITE_SHOP_DETAIL.changeContentTab(CONST.TAB_TYPE.QNA);
				else pc_tab_function(CONST.TAB_TYPE.QNA);
				if( hash_temp[1] ) {
					SITE_SHOP_DETAIL.viewQnaDetail(hash_temp[1], 1, 'Y');
				}
				break;
			case '#prod_detail_return':
				if(is_mobile_width) SITE_SHOP_DETAIL.changeContentTab(CONST.TAB_TYPE.RETURN);
				else pc_tab_function(CONST.TAB_TYPE.RETURN);
				break;
			default:
				var target_tab;
		...
		$prod_detail_content_pc.find('img').lazyload({		/* 상품 상세페이지 lazy load 적용, 기본 /img/no-image.png 는 한번만 불러옴 */
			...
			load : function(){
				...
				if($seemore_wrap.length > 0 && !is_mobile_width){
					...
					setTimeout(function(){
						if ($prod_detail_content_pc.outerHeight() < CONST.SEEMORE_HEIGHT.PC ) {
							$prod_detail_content_pc.toggleClass('hide_seemore', true);
							$seemore_wrap.hide();
						}
					}, 100);
	...
	var checkOptionCount = function(optNo, cnt) {
		...
		if ( selected_options[optNo].require ) { // 필수 옵션
			if ( prod_type === CONST.PROD_TYPE.DIGITAL ) {
	...
	var updateSelectedOptions = function(type){
		...
		if( require_option_count == 0 ) {
			...
			if(prod_type === CONST.PROD_TYPE.NORMAL){
		...
		$.each(selected_options, function(no, data){
			...
			quantity_changeable = !(prod_type !== CONST.PROD_TYPE.NORMAL && data.require == true);
	...
	var digitalFileDownload = function(order_no){
		if ( prod_type !== CONST.PROD_TYPE.DIGITAL ) return false;
		$.ajax({
			"type": "POST",
	...
	var changeContentPCTab = function(type, r_p, q_p, paging_on,only_photo,shop_view_body_width){
		...
		switch(type){
			case CONST.TAB_TYPE.DETAIL :
				...
				if($seemore_wrap.length > 0){
					if(!is_mobile_width && options.shop_pc_tab_type_one_page != 'Y'){
						$seemore_wrap.show();
						$prod_detail_content_pc.toggleClass('hide_seemore', false);
						if(!use_lazy_load || $prod_detail_content_pc.find('img').length === 0){
							if ($prod_detail_content_pc.outerHeight() < CONST.SEEMORE_HEIGHT.PC) {
			...
			case CONST.TAB_TYPE.REVIEW :
				...
				$.ajax({
					...
					success : function(result){
						...
						if(!is_mobile_width && options.shop_pc_tab_type_one_page == 'Y'){
							$prod_detail_content_pc.toggleClass('hide_seemore', false);
							if($prod_detail_content_pc.find('img').length === 0 || options.shop_pc_tab_type_one_page != 'Y'){
								if ($prod_detail_content_pc.outerHeight() < CONST.SEEMORE_HEIGHT.PC) {
									$prod_detail_content_pc.toggleClass('hide_seemore', true);
									$seemore_wrap.hide();
								}
			...
			case CONST.TAB_TYPE.RETURN:
				if ( is_init_detail ) {
					...
					if ( last_refund_data ) {
						var prod_detail_return_html = IMWEB_TEMPLATE.loadSimple("prodReturnPc", last_refund_data);
						if ( last_refund_data.use_shop_return ) {
							$_target.html(prod_detail_return_html);
						}else{
							$.ajax({
								type : 'POST',
								url : ('/shop/prod_return_disable.cm'),
								dataType : 'html',
								cache : false,
								async: false,
								success : function(result){
			...
			case CONST.TAB_TYPE.DETAIL:
						...
						$prod_detail_content_mobile.find('img').addClass('doz_lazyloading').lazyload({		/* 상품 상세페이지 lazy load 적용, 기본 /img/no-image.png 는 한번만 불러옴 */
							...
							load : function(){
								...
								if($seemore_wrap.length > 0 && is_mobile_width){
									// 레이지로드 이후 no-image보다 로딩된 이미지가 작아 펼쳐보기 높이 제한에 걸리지 않게 되면 펼쳐보기 해제
									setTimeout(function(){
										if ($prod_detail_content_mobile.outerHeight() < CONST.SEEMORE_HEIGHT.MOBILE){
											$prod_detail_content_mobile.toggleClass('hide_seemore', true);
											$seemore_wrap.hide();
										}
									}, 100);
				...
				if($seemore_wrap.length > 0){
					if(is_mobile_width){
						...
						if(!use_lazy_load || $prod_detail_content_mobile.find('img').length === 0){
							if ($prod_detail_content_mobile.outerHeight() < CONST.SEEMORE_HEIGHT.MOBILE){
								$prod_detail_content_mobile.toggleClass('hide_seemore', true);
								$seemore_wrap.hide();
							}
...
var SITE_SHOP_CART = function(){
	...
	var initCart = function(cart_code, backurl, is_regularly){
		...
		var cart_type = CONST.CART_TYPE_NORMAL;
		if(is_regularly == 'Y') cart_type = CONST.CART_TYPE_REGULARLY;
	...
	var changeCartItemComplete = function(current_ul){
		var is_regularly = 'N';
		if(cart_type == CONST.CART_TYPE_REGULARLY){
			is_regularly = 'Y';
		}
			...
			success : function(result){
				if(result.msg == 'SUCCESS'){
					...
					if(dt['is_regularly'] == 'Y'){
						cart_type = CONST.CART_TYPE_REGULARLY;
					}
					$changeCartItemLayer.hide();
		...
		$.ajax({
			...
			success : function(res){
				if(res.msg == 'SUCCESS'){
					...
					cart_type = type;
					if(type === CONST.CART_TYPE_REGULARLY){
					...
					getSelectedPrice(type === CONST.CART_TYPE_REGULARLY);
				}else
					alert(res.msg);
