...
var LIBRARY_IMAGE = function(){
	...
	var type 				= "";
	var $editor_obj, $cover_image, $represent_img, $library_img_list;
	var editor_id;
	var init = function(_type,_editor_obj,_library_img_obj){
		type = _type;
	...
	function updateLibrayImage(img_list){
		...
		if(is_update){
			var represent_img_src = $represent_img.val();
			...
			if(img_list_length > 1 || cover_image_index == -1){
				$.each(img_list,function(index,_img_src){
					if(represent_img_src === _img_src) is_represent_img = true;
				});
				if(!is_represent_img){
					$represent_img.val("");
					represent_img_src = "";
	...
	return {
		'init' : function(_type, _editor_obj, _library_img_obj) {
			init(_type, _editor_obj, _library_img_obj);
		},
